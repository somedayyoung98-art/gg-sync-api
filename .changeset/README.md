...
We use [Changesets](https://github.com/changesets/changesets) to version and publish `@somedayyoung/*` packages.
...
All `@somedayyoung/*` packages share one semver via the `fixed` group in `config.json`.
