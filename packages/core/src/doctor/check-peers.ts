...
import { loadPluginsForGenerators, type PluginLoadIssue } from './load-plugins';
    ...
    for (const [packageName, requiredRange] of Object.entries(
      ...
      if (!installed) {
        peerIssues.push({
          ...
          installHint: formatInstallHint(packageName),
        });
        continue;
      }
      ...
      if (!semver.satisfies(installed, requiredRange, { includePrerelease: true })) {
        peerIssues.push({
          ...
          installHint: formatInstallHint(packageName, { version: requiredRange }),
        });
      }
    }
  }
